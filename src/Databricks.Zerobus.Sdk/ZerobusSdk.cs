...
/// </code>
/// </example>
public sealed class ZerobusSdk : IAsyncDisposable
{
    ...
    public ZerobusSdk(string serverEndpoint, string workspaceUrl)
        : this(CreateChannel(serverEndpoint), workspaceUrl, ExtractWorkspaceId(serverEndpoint), ownsChannel: true) { }

    internal ZerobusSdk(GrpcChannel channel, string workspaceUrl, string workspaceId, bool ownsChannel)
    {
    ...
    public Task<ZerobusStream<T>> CreateStreamAsync<T>(
    ...
    public async Task<ZerobusStream<T>> CreateStreamAsync<T>(
        TableProperties<T> tableProperties,
        ITokenProvider tokenProvider,
        StreamConfigurationOptions? options = null,
        CancellationToken cancellationToken = default)
    ...
    private static GrpcChannel CreateChannel(string serverEndpoint)
    {
        if (string.IsNullOrWhiteSpace(serverEndpoint))
            throw new ArgumentException("Server endpoint is required.", nameof(serverEndpoint));
        var address = serverEndpoint.Contains("://") ? serverEndpoint : "https://" + serverEndpoint;
